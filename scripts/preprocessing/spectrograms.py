...
def write_audeep_dataset(
    ...
    label_numeric = dataset.createVariable("label_numeric", np.int64, ("instance",))
    label_nominal[:] = np.zeros(spectrograms.shape[0], dtype=str)
    label_numeric[:] = np.zeros(spectrograms.shape[0], dtype=np.int64)

    features = dataset.createVariable(
        "features", np.float32, ("instance", "time", "freq")
    )
...
def main(
    ...
    if not audeep and not netcdf:
        raise ValueError("Must specify either --preview, --netcdf or --audeep options.")
    ...
    if audeep is not None:
        ...
        write_audeep_dataset(audeep, stacked, filenames, mel_bands, corpus)
