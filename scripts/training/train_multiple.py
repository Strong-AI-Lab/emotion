...
def main(input: Tuple[Path], save: Path, cv: str, norm: str):
    ...
    param_grid = {'C': 2.0**np.arange(-6, 7, 2), 'gamma': 2.0**np.arange(-12, -1, 2)}
    clf = GridSearchCV(
        PrecomputedSVC(kernel='rbf', probability=True), param_grid,
        scoring='balanced_accuracy', cv=GroupKFold(2), n_jobs=2,
    )
    ...
    scores = cross_validate(
        ...
        fit_params={'sample_weight': sample_weight, 'groups': groups}, n_jobs=5,
        verbose=1
    )
