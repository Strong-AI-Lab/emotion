...
def test_classifier(clf: str,
    ...
    df = cross_corpus_cross_validation(classifier, dataset, reps=reps)
    for corpus in dataset.corpora:
        print("Corpus {}:".format(corpus))
        corpus_df = df.loc[corpus, :].unstack(level='rep').mean(1)
        classes = corpus_df.index.get_level_values('class').unique()
        print("UAR: {:.4f}".format(corpus_df.loc[('uar', slice(None))].mean()))
        print("WAR: {:.4f}".format(corpus_df.loc[('war', slice(None))].mean()))
        for c in classes:
            recall = corpus_df.loc[('rec', c)]
            precision = corpus_df.loc[('prec', c)]
            print("Class '{}' {:.4f} {:.4f}".format(c, recall, precision))
        print()
...
def main():
    ...
    combined = CombinedDataset(
        *[NetCDFDataset(path) for path in args.data],
        labels={'anger', 'happiness', 'sadness', 'neutral'}
    )
    combined.normalise(normaliser=StandardScaler(), scheme='speaker')
    if args.datatype == 'frame':
        combined.pad_arrays(64)

    if not args.noresults:
        resultname = args.name or args.data.stem
    else:
        resultname = None
    test_classifier(args.clf, args.kind, combined, reps=args.reps,
