...
@click.option('--learning_rate_decay', type=click.Choice(DecayType.__members__),
              default=DecayType.STEP.name)
@click.option('--momentum_decay', type=click.Choice(DecayType.__members__),
              default=DecayType.STEP.name)
@click.option('--output', type=Path)
def main(input: Path, logdir: str, batch_size: int, valid_fraction: float,
         epochs: int, init_learning_rate: float, init_momentum: float,
         final_learning_rate: float, final_momentum: float,
         learning_rate_decay: DecayType, momentum_decay: DecayType,
         output: Path):
    ...
    rbm.train(
        train_data, valid_data, n_epochs=epochs,
        init_learning_rate=init_learning_rate, init_momentum=init_momentum,
        final_learning_rate=final_learning_rate, final_momentum=final_momentum,
        learning_rate_decay=learning_rate_decay, momentum_decay=momentum_decay
    )
    if output:
        representations = rbm.representations(tf.data.Dataset.from_tensor_slices(
            spectrograms).batch(8))
        ...
        write_netcdf_dataset(output, dataset.names, representations,
                             corpus=dataset.corpus, feature_names=feature_names)
